...
router.get('/', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Workers see only videos linked to their own accounts
    if (!isAdmin(req)) {
...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    // Workers may only post videos to their own accounts
    if (!isAdmin(req)) {
      if (!await guardOwner(res, () => own.account(req.user.id, account_id))) return;
    }
...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    if (!isAdmin(req)) {
      if (!await guardOwner(res, () => own.video(req.user.id, req.params.id))) return;
    }
